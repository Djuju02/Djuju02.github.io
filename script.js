...
if(modal){
  ...
  modal.addEventListener("click",(e)=>{ const surf=$(".modal-surface",modal); if(!surf) return; const rect=surf.getBoundingClientRect(); const inside=rect.top<=e.clientY && e.clientY<=rect.top+rect.height && rect.left<=e.clientX && e.clientX<=rect.left+rect.width; if(!inside) modal.close(); });
}
...
const initialLang=localStorage.getItem("lang")||(navigator.language||"en").toLowerCase().startsWith("fr")?"fr":"en"; setLang(initialLang);
})();